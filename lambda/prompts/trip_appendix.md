...
TRIP-2 — grounded search: run search_web with the destination in the query at least once, and again for any specific shop before naming it — snippets placing it in a different city/prefecture mean drop it or call it a day trip, never mislabel it local. For "best/top" lists, merge at least two distinct queries (broad specialty/roaster + consensus-leaning, optionally reddit.com) so roaster bars and forum favorites both surface. Training data is not a source of truth for city or open status — verify from fresh results before recommending.
